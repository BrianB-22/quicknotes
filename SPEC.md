...
- Right-click a row: Lock…/Unlock…, **Remove Lock…** (locked notes only — permanently decrypts and converts back to a plain note; see Locking), Pin/Unpin, **Label** submenu (six colors — red is a plain color, *not* a "sort to top" special case, see below), Delete (confirms before deleting, same dialog as the detail-pane trash button)
...
- Bottom bar below the editor: a **TXT/MD** segmented toggle (per-note, persisted — see Storage) and, in Text mode, buttons to insert markdown syntax (Bold, Italic, Code, Heading, Bulleted List, Link) at the current selection; in MD mode the buttons are replaced with a "MD is Read-Only" label
...
- The most recently used passcode is remembered for the rest of the session (pre-fills future lock/unlock sheets) and forgotten when the popover closes — unconditionally, regardless of the auto re-lock delay setting (only the *notes themselves* stay decrypted longer under a longer delay, not the remembered passcode)
...
- Optional Touch ID: a note's passcode is saved to the Keychain (`KeychainStore.swift`) after locking, protected by `SecAccessControl(.biometryCurrentSet)` — the Secure Enclave itself enforces the biometric check on read, not just an app-side prompt beforehand. Turning the Touch ID setting off wipes every saved passcode from the Keychain. The typed passcode still works as a fallback everywhere
- **Remove Lock…**: verifies the passcode (typed or Touch ID), then permanently converts a locked note back into a normal plain-text note — clears the encrypted payload, title snapshot, and any saved Keychain entry. Until this existed there was no way back once a note was locked
...
- `Note` fields: `id`, `createdAt`, `modifiedAt`, `isPinned`, `isLocked`, `colorLabel`, `plainText` (nil while locked), `encryptedPayload` (nil while unlocked), `lockedTitleSnapshot`, `previewMode` (Text/MD, optional — nil on notes written before this field existed, treated as Text)
