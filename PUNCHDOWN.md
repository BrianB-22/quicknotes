...
### 1.10 ✅ FIXED — Reliable repro of §1.9's symptom: closing the popover from the tray icon while a sheet is open leaves it stuck presented *(user-reported, 2026-08-29)*
User found a 100%-reliable repro distinct from the general §1.9 fix: open Settings, then close the popover by clicking the tray icon (not "Done"), then reopen from the tray icon — clicks stop registering again. Root cause: `ContentView`'s `showingSettings` (and the equivalent local `@State` behind every other `.sheet`/`.confirmationDialog`/`.alert` in `NoteListView`/`NoteDetailView` — passcode prompts, Version History, delete confirmation) is never reset when the popover is force-closed. `NSPopover.performClose` tears down the popover window, but the `NSHostingController`'s SwiftUI view hierarchy (and its `@State`) persists across show/close cycles — it's created once in `AppDelegate.setupPopover()`, not recreated per show. So `showingSettings` stays `true` in memory with no window to show it in; on the next `popover.show(...)`, SwiftUI still believes a sheet is presented and keeps its modal input-capture active, even though nothing is visibly drawn — every click on the (visibly normal-looking) note list silently goes to a phantom modal instead.
**Fixed:** `NoteStore.popoverDidClose()` (already the single hook that fires on every popover close, for any reason) now bumps a new `@Published popoverCloseTick`. `ContentView`, `NoteListView`, and `NoteDetailView` each added an `.onChange(of: noteStore.popoverCloseTick)` that nils out their own local sheet/dialog state — same pattern as the existing `.onChange(of: noteStore.selectedNoteID)` in `NoteListView`. Covers Settings, both lock/unlock passcode-prompt sheets, Version History, and the delete-confirmation dialog + its follow-up alert.

### 1.11 ✅ FIXED — macOS 27 regression: popover opens and instantly closes on every click *(user-reported, 2026-09-16)*
`AppDelegate.setupPopover()`'s outside-click monitor was built on the assumption that global `NSEvent` monitors never see mouseUp events on the app's own windows (true through macOS 26) — so it only ever needed to handle clicks that land elsewhere. On macOS 27 that assumption broke: the same mouseUp that clicks the status-item button — and whose own `.leftMouseUp` action just opened the popover via `showPopover(from:)` — is now *also* delivered to the global monitor. That handler sees `popover.isShown == true` and calls `performClose` immediately, so the popover flashes open and vanishes on every click, with no error of any kind.
**Fixed:** the monitor now checks whether the mouseUp's screen location falls inside the status item button's frame and ignores it there, leaving open/close of that click entirely to the button's own action — same technique already used elsewhere to distinguish "this app's own controls" from "actually outside."
...
14. **Import notes from iPhone.** No concrete mechanism decided — candidates range from cheap (AirDrop a `.txt`/`.md` file, which the OS already routes to whatever app claims the type) to a real Shortcuts/share-sheet integration. Needs scoping before it's buildable; noted here as a want, not a spec.
15. **Dictation.** Might already work for free — `LinkAwareTextEditor` is a real `NSTextView`, and macOS's system-wide dictation (Edit menu → Start Dictation, or double-tap Fn) works in any standard text view without app-specific code. Worth just testing before assuming this needs to be built.
16. **Font selection.** Currently only Small/Medium/Large sizing (`NoteFontSize`) — no choice of typeface. Would need a system font picker or a curated list (e.g. add a monospace option alongside the default system font).
